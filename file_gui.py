...
class FxGui(Frame):
    def __init__(self, firstpeer, hops=2, maxpeers=5, serverport=5678, master=None):
        ...
        self.fxpeer.buildpeers(host, int(port), hops=hops)
        self.updatePeerList()

        t = threading.Thread(target=self.fxpeer.mainloop, args=[])
        t.start()

        self.fxpeer.startstabilizer(self.fxpeer.checklivepeers, 3)
        #      self.fxpeer.startstabilizer( self.onRefresh, 3 )
    ...
    def onAdd(self):
        ...
        if file.lstrip().rstrip():
            ...
            self.fxpeer.addlocalfile(filename)
    ...
    def onSearch(self):
        ...
        for p in self.fxpeer.getpeerids():
            self.fxpeer.sendtopeer(p,
                                   QUERY, "%s %s 4" % (self.fxpeer.myid, key))
    ...
    def onFetch(self):
        ...
        if len(sels) == 1:
            ...
            if len(sel) > 2:  # fname:host:port
                fname, host, port = sel
                resp = self.fxpeer.connectandsend(host, port, FILEGET, fname)
                if len(resp) and resp[0][0] == REPLY:
                    fd = file(fname, 'w')
                    fd.write(resp[0][1])
                    fd.close()
                    self.fxpeer.files[fname] = None  # because it's local now

    def onRemove(self):
        sels = self.peerList.curselection()
        if len(sels) == 1:
            peerid = self.peerList.get(sels[0])
            self.fxpeer.sendtopeer(peerid, PEERQUIT, self.fxpeer.myid)
            self.fxpeer.removepeer(peerid)
    ...
    def onRebuild(self):
        if not self.fxpeer.maxpeersreached():
            ...
            try:
                ...
                self.fxpeer.buildpeers(host, port, hops=3)
            except:
                if self.fxpeer.debug:
                    traceback.print_exc()


#         for peerid in self.fxpeer.getpeerids():
#            host,port = self.fxpeer.getpeer( peerid )
...
def main():
    ...
    maxpeers = sys.argv[2]
    peerid = sys.argv[3]
    app = FxGui(firstpeer=peerid, maxpeers=maxpeers, serverport=serverport)
    app.mainloop()
